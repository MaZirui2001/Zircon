...
由于访存队列是压缩的且是顺序发送的，因此寄存器堆的读地址只会来自发射队列的第0个表项，是相对固定的，故发射队列和寄存器堆之间没有段间寄存器堆。

为了实现这个功能，还需要将第一运算单元、第二运算单元和访存单元都需要将执行阶段计算出的结果前递到读数据处。考虑到读寄存器堆的时间延迟略大于ALU和数据多选器，这样的旁路网络并不会引入过多延迟。
...
为了加速运算指令的执行，这个阶段在第一和第二运算流水线中架设了旁路网络，使得写回的数据可以直接前递到执行单元，保证了相关指令可以连续发射、连续执行。
