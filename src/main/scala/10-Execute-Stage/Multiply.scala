...
class Multiply extends Module{
    ...
    def Booth2(x: UInt, y: UInt) :UInt = {
        val res = WireDefault(x)
        val neg = y(2)
        switch(y(1, 0)){
            is(0.U){ res := 0.U }
            is(3.U){ res := x(65, 1) ## 0.U(1.W) }
            is(4.U){ res := x(65, 1) ## 0.U(1.W) }
            is(7.U){ res := 0.U }
        }
        Mux(neg, ~res + 1.U, res)
    }
    def CSA(x: UInt, y: UInt, z: UInt, n: Int) : UInt  = {
        ...
        val res2 = WireDefault((((x & y) | (x & z) | (y & z)) ## 0.U(1.W))(n-1, 0))
        res2 ## res1
    }
    ...
    val booth = VecInit(Seq.tabulate(33){ i => 
        Booth2(src1(65-2*i, 0) ## 0.U((2*i).W),
               (if(i == 0) src2(1, 0) ## 0.U(1.W) else src2(2*i+1, 2*i-1))
        )
    })
    // wallce tree
    ...
    for(i <- 0 until 11){
        res1(2*i)       := level1(i)(63, 0)
        res1(2*i+1)     := level1(i)(127, 64)
    }
    ...
    for(i <- 0 until 7){
        res2(2*i)       := level2(i)(63, 0)
        res2(2*i+1)     := level2(i)(127, 64)
    }
    res2(14) := res1(21)
    ...
    for(i <- 0 until 5){
        res3(2*i)       := level3(i)(63, 0)
        res3(2*i+1)     := level3(i)(127, 64)
    }
    ...
    io.res          := Mux(io.op === ALU_MUL, res(31, 0), res(63, 32))
