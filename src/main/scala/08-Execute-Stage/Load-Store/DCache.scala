...
class DCache extends Module{
    ...
    // EX Stage
    val tagv                        = VecInit.fill(2)(Module(new xilinx_simple_dual_port_1_clock_ram_read_first(TAG_WIDTH+1, INDEX_DEPTH)).io)
    ...
    val hit_reg_EX_MEM              = ShiftRegister(VecInit.fill(5)(hit_EX), 1, TC_MEM_en)
    ...
    // MEM Stage
    val hit_MEM         = VecInit.tabulate(5)(i => VecInit.tabulate(2)(j => hit_reg_EX_MEM(i)(j)).asUInt)
    val hit_index_MEM   = VecInit.tabulate(5)(i => OHToUInt(hit_MEM(i)))
    val cache_hit_MEM   = VecInit.tabulate(5)(i => hit_MEM(i).orR)
    ...
    io.cache_miss_iq    := VecInit.tabulate(5)(i => cacop_en_MEM || mem_type_MEM_backup(i)(4, 3).orR && (uncache_MEM || !hit_MEM(i).orR))
            ...
            is(s_idle){
                when(cacop_en_MEM){
                    ...
                    when(uncache_MEM){
                        ...
                        state_backup(i)     := Mux(cache_hit_MEM(i), s_idle, s_miss)
                        cache_miss_MEM(i)   := !cache_hit_MEM(i)
                    }
                }
            }
            is(s_miss){
