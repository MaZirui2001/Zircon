...
class CRat_IO extends Bundle{
    ...
    val arch_rat     = Input(Vec(80, UInt(1.W)))
...
class CRat extends Module{
    ...
    import RAT._
    val crat = RegInit(VecInit(Seq.fill(80)(0.U.asTypeOf(new rat_t))))
    ...
    // read for rj, rk, rd
    val rj_hit_oh = Wire(Vec(4, Vec(80, Bool())))
    val rk_hit_oh = Wire(Vec(4, Vec(80, Bool())))
    val rd_hit_oh = Wire(Vec(4, Vec(80, Bool())))
    for(i <- 0 until 4){
        for(j <- 0 until 80){
