...
class CRat extends Module{
    ...
    import RAT._
    val crat = RegInit(VecInit(Seq.fill(96)(0.U.asTypeOf(new rat_t))))
    ...
    // read for rj, rk, rd
    val rj_hit_oh = Wire(Vec(4, Vec(96, Bool())))
    val rk_hit_oh = Wire(Vec(4, Vec(96, Bool())))
    val rd_hit_oh = Wire(Vec(4, Vec(96, Bool())))
    for(i <- 0 until 4){
        for(j <- 0 until 96){
